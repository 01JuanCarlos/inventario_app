    ...
    private fun loadProducts() {
        db.collection("products").get().addOnSuccessListener { result ->
            val products = result.documents.map { doc ->
                ...
                val image = doc.getString("image") ?: "" // Asegúrate de que este sea el campo correcto
                val id = doc.id // Obtén el ID del documento
                Product(id, name, quantity, price, category, image) // Crea el objeto Product
            }

            if (products.isEmpty()) {
                Toast.makeText(this, "No hay productos", Toast.LENGTH_SHORT).show()
            } else {
                Prodadapter.setProducts(products) // Asegúrate de que el adaptador esté correctamente inicializado
            }
        }.addOnFailureListener { e ->
